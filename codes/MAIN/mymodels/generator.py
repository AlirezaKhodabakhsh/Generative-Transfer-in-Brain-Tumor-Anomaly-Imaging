...
class Generator_S(nn.Module):
    ...
    features : 64
    """
    def __init__(self, features, init_size, latent_dim, channels):
        super(Generator_S, self).__init__()

        self.init_size = init_size
        self.linear_layer = nn.Sequential(nn.Linear(latent_dim, features*2 * self.init_size ** 2))
        self.features = features

        self.conv_layers = nn.Sequential(
            nn.BatchNorm2d(features*2),
            nn.Upsample(scale_factor=2),
            nn.Conv2d(features*2, features*2, 3, stride=1, padding=1),

            nn.BatchNorm2d(features*2, 0.8),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Upsample(scale_factor=2),
            nn.Conv2d(features*2, features, 3, stride=1, padding=1),

            nn.BatchNorm2d(features, 0.8),
            nn.LeakyReLU(0.2, inplace=True),
    ...
    def forward(self, z):
        ...
        out = out.view(out.shape[0], self.features*2, self.init_size, self.init_size)
